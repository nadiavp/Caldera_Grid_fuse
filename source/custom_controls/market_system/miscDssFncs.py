...
def ld_vals( DSSCircuit ):
    ii = dss.Circuit.FirstPDElement()
    S=[]; V=[]; I=[]; B=[]; D=[]; N=[]
    while ii!=0:
        if dss.CktElement.Name()[0:4].lower()=='load':
            dss.Loads.Name(dss.CktElement.Name().replace('Load.',''))
            S.append(tp_2_ar(dss.CktElement.Powers()))
            V.append(tp_2_ar(dss.CktElement.Voltages()))
            I.append(tp_2_ar(dss.CktElement.Currents()))
            B.append(dss.CktElement.BusNames())
            N.append(dss.Loads.Name())
            if B[-1][0].count('.')==1:
                D.append(False)
            else:
                D.append(dss.Loads.IsDelta())
        ii=dss.Circuit.NextPCElement() #DSSCircuit.NextPCElement()
    jj = dss.Circuit.FirstPDElement() #DSSCircuit.FirstPDElement()
    while jj!=0:
        if dss.CktElement.Name()[0:4].lower()=='capa':
            dss.Capacitors.Name(dss.CktElement.Name().replace('Capacitor.',''))
            S.append(tp_2_ar(dss.CktElement.Powers()))
            V.append(tp_2_ar(dss.CktElement.Voltages()))
            I.append(tp_2_ar(dss.CktElement.Currents()))
            B.append(dss.CktElement.BusNames())
            D.append(dss.Capacitors.IsDelta())
            N.append(dss.Capacitors.Name())
        jj=dss.Circuit.NextPCElement()
    return S,V,I,B,D,N

def find_node_idx(n2y,bus,D):
    idx = []
    BS = bus.replace('Bus.','')#split('.',1)
...
def calc_sYsD( YZ,B,I,V,S,D,n2y ): # YZ as YNodeOrder
    ...
    for i in range(len(B)):
        for busi in B[i]:
            # if there is a list of buses connected at that node, iterate
            if isinstance(busi,str): 
                busi = [busi]
            for bus in busi:
                idx = find_node_idx(n2y,bus,D[i])
                #print(f'bus: {bus}, idx: {idx}')
                BS = bus.split('.',1)
                bus_id,ph = [BS[0],BS[-1]] # catch cases where there is no phase
                if D[i]:
                    if bus.count('.')==2:
...
def get_sYsD(DSSCircuit):
    S,V,I,B,D,N = ld_vals(dss.Circuit) # DSSCircuit )
    n2y = node_to_YZ(dss.Circuit) #DSSCircuit)
    V0 = tp_2_ar(dss.Circuit.YNodeVArray())*1e-3 #DSSCircuit.YNodeVArray())*1e-3 # kV
    YZ = dss.Circuit.YNodeOrder() #DSSCircuit.YNodeOrder()
    iY, sY, iD, sD = calc_sYsD( YZ,B,I,V,S,D,n2y )
    H = create_Hmat(dss.Circuit) #DSSCircuit)
    H = H[iD.nonzero()]
    sD = sD[iD.nonzero()]
...
def create_Hmat(DSSCircuit):
    n2y = node_to_YZ(dss.Circuit) #DSSCircuit)
    Hmat = np.zeros((dss.Circuit.NumNodes(), dss.Circuit.NumNodes())) #np.zeros((DSSCircuit.NumNodes(),DSSCircuit.NumNodes()))
    for bus in dss.Circuit.AllBusNames(): #DSSCircuit.AllBusNames():
        #print(f'bus_hmat = {bus}')
        dss.Circuit.SetActiveBus(bus)
        phases = dss.Bus.Nodes()
        bus = bus+'.'+'.'.join([str(phase) for phase in phases])
        #print(f'bus_hmat = {bus}')
        # find the index of each phase
        idx_list = find_node_idx(n2y,bus,False)
        # convert to include nones for missing phases
        idx = [None, None, None]
        j = 0
        for idxi in idx_list:
            idx[j] = idxi
            j = j+1
        # incorporate relationships into H matrix
        Hmat[idx[0], idx[0]] = 1
...
def cpf_get_loads(DSSCircuit):
    ...
    i = dss.Loads.First()
    while i!=0:
        SS[i]=dss.Loads.kW() + 1j*dss.Loads.kVABase()
        BB[i]=dss.Loads.Name()
        i=dss.Loads.next()
    imax = dss.Loads.Count()
    j = dss.Capacitors.First()
    while j!=0:
        SS[imax+j]=1j*dss.Capacitors.kvar()
        BB[imax+j]=dss.Capacitors.Name()
        j = dss.Capacitors.Next()
    return BB,SS
...
def cpf_set_loads(DSSCircuit,BB,SS,k):
    ...
    while i!=0:
        ...
        dss.Loads.kW(k*SS[i].real)
        dss.Loads.kvar(k*SS[i].imag)
        i=dss.Loads.Next()
    imax = dss.Loads.Count()
    j = dss.Capacitors.First()
    while j!=0:
        dss.Capacitors.Name(BB[j+imax])
        dss.Capacitors.kvar(k*SS[j+imax].imag)
        j=dss.Capacitors.Next()
    return
...
def create_tapped_ybus_very_slow( DSSObj,fn_y,TC_No0 ):
    dss.Command('Compile ('+fn_y+')')
    fix_tap_pos(dss.Circuit, TC_No0)
    dss.Command('set controlmode=off')
    dss.Solution.Solve()
    
    SysY = dss.Circuit.SystemY()
    SysY_dct = {}
    i = 0
    ...
    Ybus = Ybus_[3:,3:]
    YNodeOrder_ = dss.Circuit.YNodeOrder()
...
def build_y(DSSObj,fn_ckt):
    ...
    YNodeOrder = dss.Circuit.YNodeOrder()
    DSSObj.Text.command='show Y'
    os.system("TASKKILL /F /IM notepad.exe")
    
    fn_y = fn_ckt+'\\'+dss.Circuit.Name()+'_SystemY.txt'
    fn_csv = fn_ckt+'\\'+dss.Circuit.Name()+'_SystemY_csv.txt'
...
def get_idxs(e_idx,DSSCircuit,ELE):
    i = ELE.First
    while i:
        for BN in dss.CktElement.BusNames:
            splt = BN.upper().replace('Bus.','')#split('.')
            if len(splt) > 1:
