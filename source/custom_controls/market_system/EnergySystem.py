...
#from System.Network3phpf import Network_3ph
import cvxopt
...
class EnergySystem:
    ...
    storage_assets : list of objects
        Containing details of each stationary storage asset
    evse_assets : list of objects
        Containing details of each EVSE and limits from
            vehicles charging at that EVSE
...
#######################################
    def EMS_copper_plate(self):
        ...
        Output : dictionary
            The following numpy.ndarrays are present depending upon asset mix:
                ...
                P_EVSE_val : Charge/discharge power for plugged in vehicles (kW)
        ...
        N_INDEPENDENT = N_ES + N_BLDG + N_EVSE
        ...
        for i in range(N_ES):
            ...
            #minimum energy constraint: sum of powerdt >= Emin-E0
            prob.add_constraint(self.dt_ems*Asum*P_ctrl_asset[:,N_BLDG+i] >=\
                                self.storage_assets[i].Emin\
                                -self.storage_assets[i].E0)
            #final energy constraint: conservation of energy
            # sum of powerdt = ET-E0
            prob.add_constraint(self.dt_ems*Asum[self.T_ems-1,:]\
                                *P_ctrl_asset[:,N_BLDG+i] ==\
                                self.storage_assets[i].ET\
                                -self.storage_assets[i].E0)

        #linear evse model constraints
        for i in range(N_EVSE):
            #maximum power constraint
            prob.add_constraint(P_ctrl_asset[:,N_BLDG+N_ES+i] <=\
                                self.evse_assets[i].Pmax)
            #minimum power constraint
            prob.add_constraint(P_ctrl_asset[:,N_BLDG+N_ES+i] >=\
                                self.evse_assets[i].Pmin)
            #maximum energy constraint
            prob.add_constraint(self.dt_ems*Asum*P_ctrl_asset[:,N_BLDG+N_ES+i] <=\
                                self.evse_assets[i].Emax\
                                -self.evse_assets[i].E0)
            #minimum energy constraint
            prob.add_constraint(self.dt_ems*Asum*P_ctrl_asset[:,N_BLDG+N_ES+i] >=\
                                self.evse_assets[i].Emin\
                                -self.evse_assets[i].E0)
            #final energy constraint: must have atleast desired SOC
            # sum of P[charge duration] dt >=
            for charge_sess in len(self.evse_assets[i].ET):
                plugin_t = self.evse_assets[i].t_plugin[charge_sess]
                plugout_t = self.evse_assets[i].T[charge_sess]
                prob.add_constraint(self.dt_ems*Asum[plugin_t:plugout_t,:]\
                                    *P_ctrl_asset[plugin_t:plugout_t,N_BLDG+N_ES+i] >=\
                                    self.storage_assets[i].ET[charge_sess]\
                                    - self.storage_assets[i].E0[charge_sess])
        ...
        output = {'P_import_val': P_import_val,\
        ...
        if N_ES > 0:
            output['P_ES_val'] = P_ctrl_asset_val[:,N_BLDG:N_ES+N_BLDG]
        if N_EVSE > 0:
            output['P_EVSE_val'] = P_ctrl_asset_val[:, N_ES+N_BLDG:]
        #if len(P_ctrl_asset) == 0:
    ...
    def simulate_network(self):
        ...
        N_nondispatch = len(self.nondispatch_assets) #number of passive loads
    ...
    def simulate_network_3phPF_greedy(self, ems_type = 'greedy',
        ...
        ### STEP 1: solve the optimisation
        #######################################
        t0 = 0  # Open-loop based optimization
        if ems_type == 'copper_plate':
            output_ems = self.EMS_copper_plate_t0(t0)  # 这个函数 may be able to be used for the site EMS in future
        else:
            output_ems = self.EMS_3ph_linear_t0_greedy(t0,
                                                i_unconstrained_lines,
                                                v_unconstrained_buses)
...
#                              'd')
    ...
    # not consider grid operation constraints.
    def EMS_3ph_linear_t0_greedy(self, t0, i_unconstrained_lines=[],
                          v_unconstrained_buses = []):
        """
        Energy management system optimization assuming 3 phase linear network
        model for Model Predictive Control interval t0
        ...
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # 修改目标函数，脱离实际的cost 意义，penalize late charging behaviors 
        # 以实现greedy charging （本质上是一种uncontrolled charging）
        
        #prices_export = 0.05*np.ones(T_market)  #(£/kWh)
        # 修改为一个penalty factor！
        penalty_factor = 1*np.arange(T_mpc) 

        prob.set_objective('min',  
                            sum(self.dt_ems*penalty_factor[t]*P_import[t]\
                              for t in range(T_mpc))\
                            + sum(terminal_const*E_T_min[i]\
                                  for i in range(N_ES)))
