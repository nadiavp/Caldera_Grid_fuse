...
class StorageAsset(Asset):
    ...
    """
    def __init__(self, Emax, Emin, Pmax, Pmin, E0, ET, bus_id, dt=5*60, T=24*60*60, dt_ems=5*60,
                 T_ems=24*60*60, phases=[0, 1, 2], Pmax_abs=None, c_deg_lin=None,
                 eff=1, eff_opt=1):
...
class StorageAsset_3ph(Asset_3ph):
    ...
    """
    def __init__(self, Emax, Emin, Pmax, Pmin, E0, ET, bus_id, phases=[0, 1, 2], 
                 dt=5*60, T=24, dt_ems=5*60,
                 T_ems=24, Pmax_abs=None, c_deg_lin=None,
                 eff=1, eff_opt=1):
...
class Evse(StorageAsset):
    # this class is almost the same as storage assets, but is a different class
    # so that it can be controlled separately from stationary storage
    # it also has default values
    # one EVSE object should be added per EVSE simulated
    # this only manages one plug
    # values are in kW, kWh, or seconds
    def __init__(self, Emax=[150]*24, Emin=[0]*24, Pmax=[11.2]*24, Pmin=[0]*24, E0=[0]*24, ET=[0], bus_id='', phases=[0], 
                 dt=5*60, T=[24*12], dt_ems=5*60,
                 T_ems=24*60*60, Pmax_abs=None, c_deg_lin=None,
                 eff=1, eff_opt=1, evse_type='L2', t_plugin=[0]):
        # Emin, Emax, Pmin, Pmax are lists for each step in the horizon
        # will get updated in accordance with any vehicle at the station
        # E0 should be the SOC of the vehicle at the current time and is a list of
        # ET should be SOC of the vehicle at the unplug time and is a list to accommodate multiple plug session
        # T will be the unplug times and is a list to accommodate multiple plug sessions
        # all the above described inputs should be lists with one entry per vehicle in the horizon
        # these should capture the individual vehicles that arrive at the location
        # if there are no vehicles arriving at the station within the horizon, 
        # then the Pmax, Pmin, Emax, Emin, E0 and ET all equal 0
        # t_plugin is a list of plugin times with the corresponding T plug-out times
