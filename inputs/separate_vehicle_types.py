...
def update_controls_by_vehicle_type(feeder_name='Hanover_01359'):
...
if __name__ == "__main__":
    # get all the CE and SE files in the inputs dir and update them
    for file in os.listdir('.'):
        if file.startswith('CE_') and file.endswith('.csv'):
            feeder_name = file.replace('CE_','').replace('.csv','').replace('longdwell_','').replace('Sep_','')
            update_controls_by_vehicle_type(feeder_name=feeder_name)
            print(f'updated CE file for {feeder_name}')